...
NS_LOG_COMPONENT_DEFINE ("WifiBianchiDsssValidation");
...
int main (int argc, char *argv[])
{
  ...
  cwTraceFile.open ("wifi-dsss-cw-trace.out");

  // Align with Bianchi DSSS values
  Config::SetDefault ("ns3::DcaTxop::MinCw", UintegerValue (31));
  ...
  std::stringstream ss;
  ss << "wifi-dsss-"<<netSize<<"-p-"<<pktSize<<"-throughput.plt";
  std::ofstream netSizeThroughputPlot (ss.str().c_str());
  ss.str("");
  ss << "wifi-dsss-"<<netSize<<"-p-"<<pktSize<<"-throughput.eps";
  ...
  experiment = Experiment ("1mb/s");
  wifi.SetRemoteStationManager ("ns3::ConstantRateWifiManager",
                                "DataMode", StringValue ("DsssRate1Mbps"),
                                "ControlMode", StringValue ("DsssRate1Mbps"));
  for (uint32_t n = 5; n <= netSize; n += 5)
    {
      std::cout << "Running experiment for " << n << " nodes " << std::endl;
      cwTraceFile << "# " << n << " nodes" << std::endl;
