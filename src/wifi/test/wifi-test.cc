...
{
  // Create ListErrorModel to corrupt ADDBA req packet. Please note that since
  // ListErrorModel uses packet UID to filter packet, and UID is incremented
  // between simulations, this test suite may fail if the tests before this
  // is modified.
  Ptr<ListErrorModel> staPem = CreateObject<ListErrorModel> ();
  std::list<uint32_t> blackList;
  // Block ADDBA request (UID 1262)
  blackList.push_back (1262);
  staPem->SetList (blackList);

  {
    RunSubtest (PointerValue (), PointerValue (staPem));
    NS_TEST_ASSERT_MSG_EQ (m_droppedActionCount, 7, "ADDBA request packet is not dropped");
  ...
  Ptr<ListErrorModel> apPem = CreateObject<ListErrorModel> ();
  blackList.clear ();
  // Block ADDBA response (UID 1334)
  blackList.push_back (1334);
  apPem->SetList (blackList);

  {
    RunSubtest (PointerValue (apPem), PointerValue ());
    NS_TEST_ASSERT_MSG_EQ (m_droppedActionCount, 7, "ADDBA response packet is not dropped");
    NS_TEST_ASSERT_MSG_EQ (m_receivedNormalMpduCount, 6, "Receiving incorrect number of normal MPDU packet on subtest 1");
    NS_TEST_ASSERT_MSG_EQ (m_receivedAmpduCount, 4, "Receiving incorrect number of AMPDU packet on subtest 1");
  }
}
